...
Cog V2 API (run method). Weights pre-downloaded at build time.
"""
...
KONTEXT_PATH = "./models/kontext/kontext-dev.sft"
AE_PATH = "./models/flux-dev/ae.safetensors"
T5_PATH = "./models/t5"
CLIP_PATH = "./models/clip"


def build_prompt(colorize: bool) -> str:
    p = (
        "Restore this old photograph. "
        "Fix all scratches, tears, dust spots, creases, and physical damage. "
        "Remove noise and grain. "
        "Enhance facial details, sharpen edges, improve overall clarity. "
        "Preserve the original composition, subjects, and lighting direction exactly. "
    )
    if colorize:
        p += (
            "Colorize naturally with realistic skin tones, natural fabric colors, "
            "and appropriate background hues. Look like an authentic color photograph."
        )
    else:
        p += "Keep original black-and-white tones."
    return p


class Predictor(BasePredictor):
    """Photo Restore with FLUX.1 Kontext"""
    ...
    def setup(self) -> None:
        ...
        print(f"T5: {time.time()-t0:.1f}s, GPU {torch.cuda.memory_allocated()/1e9:.1f}GB", flush=True)

        t0 = time.time()
        print("Loading CLIP...", flush=True)
        self.clip = load_clip(self.device, clip_path=CLIP_PATH)
        print(f"CLIP: {time.time()-t0:.1f}s, GPU {torch.cuda.memory_allocated()/1e9:.1f}GB", flush=True)

        t0 = time.time()
        print("Loading FLUX Kontext...", flush=True)
        cfg = configs["flux-dev"]
        with torch.device("meta"):
            self.model = Flux(cfg.params).to(torch.bfloat16)
        sd = load_sft(KONTEXT_PATH, device=str(self.device))
        self.model.load_state_dict(sd, strict=False, assign=True)
        self.model.eval()
        print(f"Kontext: {time.time()-t0:.1f}s, GPU {torch.cuda.memory_allocated()/1e9:.1f}GB", flush=True)

        t0 = time.time()
        print("Loading AE...", flush=True)
        with torch.device("meta"):
            self.ae = AutoEncoder(cfg.ae_params)
        sd = load_sft(AE_PATH, device=str(self.device))
        self.ae.load_state_dict(sd, strict=False, assign=True)
        print(f"AE: {time.time()-t0:.1f}s, GPU {torch.cuda.memory_allocated()/1e9:.1f}GB", flush=True)
    ...
    def run(
        ...
        prompt = build_prompt(auto_colorize)
        print(f"Restoring: colorize={auto_colorize}, guidance={guidance}", flush=True)
        ...
        with torch.inference_mode():
            ...
            t0 = time.time()
            cmap = generate_compute_step_map("go really fast", 28)
            timesteps = get_schedule(28, inp["img"].shape[1], shift=True)
            x = denoise(self.model, **inp, timesteps=timesteps, guidance=guidance, compute_step_map=cmap)
